 ...
 *   pnpm phase4:widgets -- --live             # regenerate every widget (or LIVE=1)
 *   pnpm phase4:widgets -- --live thr-006     # regenerate one (the finale, on stage)
 *
 * This is the API-key route. The AI-CLI route does not use this script: your
 * agent writes widgets.json directly from the objective in docs/phase-4.md, then
 * `pnpm phase4:check` validates it.
 *
...
async function main() {
  ...
  const live = llm !== null && (process.env.LIVE === '1' || process.argv.includes('--live') || !hasCache);
  ...
  if (!live) {
    if (!hasCache) {
      console.error('No cached widgets and no model configured. Set LLM_API_KEY (see README), or use your AI CLI (see docs/phase-4.md).');
      process.exit(1);
    }
  ...
  console.log(`\nWrote ${cachePath}`);
  printCheck(
    'Phase 4 · widgets.json (fresh)',
    validateWidgets({ raw: result, threads, comments, files, primitives: Object.keys(PRIMITIVES) }),
  );
}
